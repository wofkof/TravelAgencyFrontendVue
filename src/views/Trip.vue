<template>
  <div class="max-w-7xl mx-auto p-4">
    ...
    <div class="bg-gray-50 rounded-xl p-4 shadow-lg mb-6">
      ...
      <!-- 標題區塊 -->
      <div
        class="rounded-lg p-4 mb-4 text-gray-800 font-semibold text-left text-2xl"
      >
        {{ mainInfo.title }}
      </div>
      ...
      <div class="flex h-96 rounded-lg">
        ...
        <div class="w-1/2 flex gap-2 p-4 text-left relative">
          <!-- 聖凱新增 -->
          <div class="absolute top-0 right-0 z-10" @click.stop>
            <OfficialCollectIcon :travelId="mainInfo.projectId" />
          </div>
          ...
          <div class="w-1/2 rounded-md">
            <img
              :src="mainInfo.cover"
              alt=""
              class="h-56 w-full object-cover rounded"
            />
          </div>
          <div class="text-lg font-bold my-2">
            <p>行程 {{ mainInfo.number }}</p>
            <p>
              可賣： {{ mainInfo.availableSeats }} 席次：{{
                mainInfo.totalSeats
              }}
            </p>
          ...
          <cartButton
            ...
            :availableSeats="
              selectedGroup?.availableSeats || mainInfo.availableSeats
            "
    ...
    <div
      class="bg-white rounded-xl p-4 shadow mb-6"
                ...
                <div>
                  ...
                  >
                    ...
                    <template #default>
                      ...
                      <div
                        :id="`map-popover-${s.scheduleId}-${a.attractionId}`"
...
const handleAddToCart = () => {
  ...
  const availableSeatsForSelectedGroup =
    selectedGroup.value?.availableSeats || mainInfo.value.availableSeats || 0;
  ...
  const productToAdd = {
    ...
    totalDays:
      selectedGroup.value.departure && selectedGroup.value.return
        ? Math.ceil(
            (new Date(selectedGroup.value.return) -
              new Date(selectedGroup.value.departure)) /
              (1000 * 60 * 60 * 24)
          ) + 1
        : mainInfo.value.days || 0, // 簡化天數計算或從 mainInfo 獲取
    departureDate: formatDateTime(selectedGroup.value.departure, {
      type: "date",
    }), // 用於過期判斷
  ...
  try {
    ...
    console.log(
      "加入購物車的官方行程:",
      JSON.parse(JSON.stringify(productToAdd))
    );
...
const handleGroupClick = async (row) => {
  ...
  ElMessage({
    message: `已選擇${formatDateTime(mainInfo.value.departure, {
      type: "date",
    })}出發的行程`,
    type: "success",
  });
    ...
    setTimeout(() => {
      for (const s of scheduleList.value) {
        for (const a of s.attractions) {
          ...
          if (popoverEl) {
            const popoverMap = L.map(popoverMapId).setView(
...
const getAttractionById = async (attractionId) => {
  ...
  try {
    const attrlist = await api.get(
      `/OfficialSearch/getAttraction/${attractionId}`
    );
...
onMounted(async () => {
  ...
  try {
    const res = await api.get(
      `/OfficialSearch/getMainInfo/${projectId}/${detailId}/${groupId}`
    );
